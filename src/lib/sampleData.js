...
export function sampleProjects() {
  ...
  const todo = (text, done = false, dept = '', due = '') => ({
    id: createId('t'),
    text,
    done,
    dept,
  ...
  return [
    {
      ...
      todos: [
        todo('客先仕様の最終確認', true, 'sales'),
        todo('架台図面の出図', false, 'mecha', d(6)),
        todo('制御盤の部品手配', false, 'elec', d(12)),
        todo('取扱説明書ドラフト作成', false, 'cs'),
      ],
    },
    {
      name: '搬送コンベア更新',
      ...
      todos: [todo('現地調査（レイアウト実測）', true, 'sales'), todo('見積再提出', false, 'sales', d(4))],
    },
    {
      name: '包装機オプション追加',
      client: 'みなみ食品',
      ...
      todos: [todo('オプション仕様のヒアリング', false, 'sales', d(3))],
    },
    {
      name: '試験機 レトロフィット',
      client: '北陸テック',
      ...
      todos: [todo('検収書の受領', true, 'sales'), todo('保守部品リスト送付', true, 'cs')],
      completedAt: d(-38),
    },
  ].map((p) => normalizeProject({ ...p, id: createId() }));
}
